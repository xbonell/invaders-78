...
- **Steam Deck desktop layout:** A→Enter, Y→Space, B/Menu→Esc. Enter fires while playing; **Space fire is suppressed while any gamepad is connected** so Y does not steal fire after fullscreen remaps to desktop keys.
