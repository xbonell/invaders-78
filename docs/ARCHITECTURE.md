...
- **Gamepad:** poll **all** connected pads (OR inputs). On `"standard"` mapping use D-pad buttons 12–15 + sticks only — do **not** read axes 6/7 (POV idle values >1 cancel steer). Non-standard mapping may use digital hat axes in [-1,1]. Fullscreen toggles refocus the page; keyboard `blur` must not clear `moveDir` unless move keys were held (focus flicker vs pad). Do not await audio unlock before pad fire/Start (same as keyboard).
- **Steam Deck desktop layout:** A→Enter, Y→Space, B/Menu→Esc. Enter must fire while playing (not only confirm start) so A matches South; Esc pauses/resumes; pad Start (button 9) also pauses when Steam exposes the Menu button as a gamepad.
