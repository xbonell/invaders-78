...
import { alienWorldPos, createBunkers, createFormation } from './formation';
import type { Alien, Ufo } from './types';

function alien(partial: Pick<Alien, 'id' | 'col' | 'row' | 'type'> & { alive?: boolean }): Alien {
...
describe('demoAi', () => {
  it('prefers squid over a nearer octopus', () => {
    ...
    const { aimX, found } = pickDemoAim([octopus, squid], formation, octX, PLAYER.z, null, [], 0);
  ...
  it('prefers UFO over aliens when the lane is clear', () => {
    ...
    const { aimX, found } = pickDemoAim([squid], formation, 0, PLAYER.z, ufo, [], 0);
    expect(found).toBe(true);
    expect(aimX).toBe(3);
  });
  ...
  it('does not request fire when far from aim X', () => {
    expect(demoShouldFire(0, 2, 5, PLAYER.z, [], false, true, ATTRACT.demoAlignTol)).toBe(false);
  });

  it('requests fire when aligned on a clear lane', () => {
    expect(demoShouldFire(1, 1.1, 5, PLAYER.z, [], false, true, ATTRACT.demoAlignTol)).toBe(true);
  });
  ...
  it('does not fire through a bunker even when aligned', () => {
    ...
    expect(demoShouldFire(x, x, 10, PLAYER.z, bunkers, false, true)).toBe(false);
  });
